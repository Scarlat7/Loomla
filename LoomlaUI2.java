...
public class LoomlaUI extends javax.swing.JFrame {
    ...
    private void LogButtonActionPerformed(java.awt.event.ActionEvent evt) {
        ...
        User actual = new User(0, "Scarlat7", "");//searchTrie(username);
          
        if(pass.equals(actual.senha)){
            PasswordField.setText("");
            LoginText.setText("");
            UserButton.setText(actual.nome);
            UserButton2.setText(actual.nome);
    ...
    private void TabelaPalavrasMouseClicked(java.awt.event.MouseEvent evt) {
        ...
        if(name.equals("Vezes")){
    		Integer[] s = (Integer[]) getCol(TabelaPalavras, col);
    		//MergeSortNumer(s, 0, 0, ordemPalavra[col]);
    		for(int i = 0; i<s.length; i++)
    			for(int j = 0; j<TabelaPalavras.getColumnCount(); j++)
    				TabelaPalavras.setValueAt(s[i], i, j);
    		
        }else{
        	String[] s = Arrays.copyOf(getCol(TabelaPalavras, col), TabelaPalavras.getColumnCount(), String[].class);
        	//String[] s = getCol(TabelaPalavras, col);
        	for(int i = 0; i<s.length; i++)
        		System.out.println(s[i]);
    		MergeSortLexic(s, 0, 0, ordemPalavra[col]);
    		for(int i = 0; i<s.length; i++){
    				TabelaPalavras.setValueAt(s[i], i, j);
    		}
        }
    ...
    private void TabelaTextosMouseClicked(java.awt.event.MouseEvent evt) {
        ...
        if(name.equals("Entendimento")){
    		Integer[] s = (Integer[]) getCol(TabelaTextos, col);
    		//MergeSortNumer(s, 0, 0, ordemPalavra[col]);
    		for(int i = 0; i<s.length; i++)
    			TabelaPalavras.setValueAt(s[i], i, col);
        }else{
        	String[] s = (String[]) getCol(TabelaTextos, col);
    		MergeSortLexic(s, 0, 0, ordemPalavra[col]);
    		for(int i = 0; i<s.length; i++)
    			TabelaTextos.setValueAt(s[i], i, col);
        }
    }
   ...
   private static void combineLexic(String[] v, int begin, int mid, int end, boolean order){

    	int i, j, k;
    	int quantity = end - begin;               

    	String[] aux = new String[quantity];
    ...
    private static void MergeSortLexic(String[] s, int inicio, int ultimo, boolean ordem){
    	
        if(inicio < ultimo-1){
          int mid = (inicio + ultimo)/2;        
          MergeSortLexic(s, inicio, mid, ordem);   
          MergeSortLexic(s, mid, ultimo, ordem);    
          combineLexic(s, inicio, mid, ultimo, ordem);       
       }
    }
